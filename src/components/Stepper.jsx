    ...
    export default function Stepper() {
      ...
      return (
        <div className="relative">
          ...
          <div 
            ref={containerRef} 
            className="overflow-x-auto overscroll-x-contain -mx-4 px-4 snap-x snap-mandatory relative z-10" 
            style={{ WebkitOverflowScrolling: 'touch' }}
            aria-label="Booking steps"
          >
            <ol className="flex items-center gap-2 sm:gap-4 min-w-[720px] py-1">
                  ...
                  <li key={s.label} className="flex items-center snap-center">
                    <button
                      ...
                      className={classNames(
                        'flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-3 rounded-2xl transition-all duration-200',
                        active ? 'gradient-bg text-white shadow-lg scale-105' : completed ? 'bg-green-100 text-green-700 border-2 border-green-200' : 'bg-white text-slate-600 border-2 border-slate-200 hover:border-blue-300',
                    ...
                    >
                      <div className={classNames(
                        ...
                        active ? 'bg-white/20' : completed ? 'bg-green-600 text-white' : 'bg-slate-100'
                      )}>
                        <Icon size={16} className={active ? 'text-white' : ''} />
                      </div>
                      <span className="text-sm sm:text-base font-semibold whitespace-nowrap">{s.label}</span>
                    </button>
                    {idx < visibleSteps.length - 1 && (
                      <span className="hidden sm:block mx-2 h-0.5 w-8 bg-slate-300" />
                    )}
                  </li>
                )
              })}
